...
char * state_get (int state) {
  switch (state) {
    case ual_STATE_INITIAL:                                           // 0
      return ("ual_STATE_INITIAL");
    case ual_STATE_STARTIN:                                           // 1
      return ("ual_STATE_STARTIN");
    case ual_STATE_STARTED:                                           // 2
      return ("ual_STATE_STARTED");
    case ual_STATE_STOPPIN:                                           // 3
      return ("ual_STATE_STOPPIN");
    case ual_STATE_STOPPED:                                           // 4
      return ("ual_STATE_STOPPED");
  }
  return ("ual_STATE Unknown error");
}
#endif
...
int ual_log (int prio, const char * tag, const char * func, const char * fmt, ...) {
  ...
  if (! ena_log_extra && prio == ual_LOG_EXT)
    return -1;
  if (! ena_log_verbo && prio == ual_LOG_VER)
    return -1;
  if (! ena_log_debug && prio == ual_LOG_DEB)
    return -1;
  if (! ena_log_warni && prio == ual_LOG_WAR)
    return -1;
  if (! ena_log_error && prio == ual_LOG_ERR)
    return -1;
